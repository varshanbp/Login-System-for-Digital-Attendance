...
<!--PHP CODE - START-->
  ...
  if(!$phash) {
    ...
    <footer class='w3-container w3-teal'>
      <p>&copy; 2016 JSSV under Attribution Assurance License. All rights reserved</p>
    </footer>
  ...
  if(password_verify($pwd, $phash['pin_hash'])) {
    $sql_arr2=mysqli_query($sql_conn, "SELECT validate, department FROM auth WHERE uid='$uid'");
    $res_arr1=mysqli_fetch_assoc($sql_arr2);
    $dept=$res_arr1["department"];
    $vstr=$res_arr1["validate"];
    if($vstr=="N") {
      echo "<p class='w3-xlarge' style='text-align: center;'>Your account has not validated.</p>";
    }
    echo "<p class='w3-xlarge' style='text-align: center;'>Successfully Authenticated. Please wait while we redirect you to portal .....</p>";
    session_start();
    $_SESSION["uid"] = $uid;
    $_SESSION["dept"] = $dept;
    $url='http://jssv.gq/mynew/index.php?dept='.$dept;
